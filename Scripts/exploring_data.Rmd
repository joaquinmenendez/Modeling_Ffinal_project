...
tapply(multiple_data$dprime, multiple_data$SOA, shapiro.test) #Da normal!!!!psych::describe(multiple_data[multiple_data$SOA== 66,]$dprime)
...
Variables are homogeneous (Levene Test F = 2.0749, p = 0.1086), and groups follow a normal distribution (Shapiro Wilk test, ps > .05 for all groups).
There is statistical evidence of a diference among dprime per group F(3,96)=22.29 p < .001.
The bigger the SOA, the bigger the dprime. It could be interpretated as the  increase os SOA  facilitates the concious processing of the prime stimuus.
...
Prelikert by SOA groups are homogenous (Levene test  F = 0.1087, p = 0.9548) but not follow a normal distribution (Shapiro Wilk test, ps < .01). Given that reason, I decided to apply a non-parametric Kruskall Wallis. The Kruskall test show a statistical trend ($X^2$= 6.575, p = .079). The Wilcoxoon rank sum test shows that there are no difference between groups. We could claim that there are no evidence of difference among groups for prelikert values.
...
Given this information we could affirm that there is statistical evidence to affirm that the different SOA interval has a diferential effect over the conciouss processing of the prime stimuli during the objective visibility test.
If we take in count the evidence of the subjective report,  there is no evidence to claim that the suggested processed diferentially the prime stimulus during the experimental task (priming task). Given that the attention has a crucial role on conciouss processing, it could be that the short period of time (even the bigger SOA interval) combined with the focus on the processing of the target stimuli is interfering on the processing of the prime stimuli. 
...
#ggplot(data = df_summarized) + geom_jitter(aes(x=dprime, y=RT, color = SOA))+  theme_classic() + facet_wrap(~SOA)

#General linear relation
ggplot(data = df_summarized,aes(x=dprime, y=RT, color = Relation, fill = Relation)) + geom_point()+ geom_smooth(method = lm, alpha=0.2)  +  theme_classic()

ggplot(data = df_summarized,aes(x=prelikert, y=RT, color = Relation, fill = Relation)) + geom_point()+ geom_smooth(method = lm, alpha=0.2)  +  theme_classic()

# Linear relations by group
ggplot(data = df_summarized,aes(x=dprime, y=RT, color = Relation, fill = Relation)) + geom_point()+ geom_smooth(method = lm, alpha=0.2) + facet_wrap(~SOA) +  theme_classic()

ggplot(data = df_summarized,aes(x=prelikert, y=RT, color = Relation, fill = Relation)) + geom_point()+ geom_smooth(method = lm, alpha=0.2) + facet_wrap(~SOA) +  theme_classic()
```
...
histogramRT<-ggplot(df_summarized, aes(x=RT, color = SOA , fill = SOA))+
  labs(x="RT")+
  ggtitle("Density plot of RT (untransformed)") +
  geom_density( )+
  stat_function(fun = dnorm, colour = "Black" , size = 1, args = list(mean=mean(df_summarized$RT), sd=sd(df_summarized$RT))) + facet_wrap(~SOA)  + theme_classic()
histogramRT
...
When we checked the normality on the RT variable for the treatment groups we observed that the groups are not homogeneous (Levene test F(3,296)= 8.956, p < .001) and also dont follow a normal distribution (Shapiro Walk test, ps < .05 for all groups)
...
#Checking distribution Response time
histogramRT<-ggplot(df_transf, aes(x=RT, color = SOA , fill = SOA))+
  labs(x="RT")+
  ggtitle("Density plot of log(RT)")+
  geom_density()+
  stat_function(fun = dnorm, colour = "Black", size = 1, args = list(mean=mean(df_transf$RT), sd=sd(df_transf$RT))) + facet_wrap(~SOA)  + theme_classic()
histogramRT
#Not changes
```
...
histogramRT<-ggplot(df_inv, aes(x=RT, fill = SOA))+
  ...
  geom_density(color = 'black')+
  stat_function(fun = dnorm, colour = "Black", size = 1, linetye= 2, args = list(mean=mean(df_inv$RT), sd=sd(df_inv$RT))) + facet_wrap(~SOA)  + theme_classic() 
histogramRT
...
I tried to apply logaritmic and inverse transformation to the data in order to homogenize the data. Despiste that the variance among groups remained different. The original intention was to realize a Mixed design ANOVA 4x3 where:
IV between: SOA
IV within:  Relation
DV:         RT

Given the urrents limitations, I would use a multilevel model for repeated mesures. This model would allow to work with normally distributed data given the fact that it will not be necessary to average the RT of each subject per condition, allowing to work with all the trials of each subject. 
=This would allow to respond the answer of the role of the treatment group an the possible interactions with the semantic relatedness. 
...
#### or an alternative is to apply a rank to the VD and perform an ANOVA
rRT = rank(s66$RT)
a66 = aov(rRT ~ Relation + Error(ID/Relation), data =s66)
summary(a66)
ao66 = lmer(rRT~Relation + (1|ID), data = s66)
anova(ao66)
#############
...
ggplot(data= dffinal, aes(x=Relation, y=RT, fill = Relation)) + geom_bar(position = 'dodge', stat = 'summary' ,color= 'black') + facet_wrap(~SOA) + theme_classic() + coord_cartesian(ylim = c(0.4, 0.6))

```

 analysis don't show statistical evidence of difference by Relation condition.
...
#The data suggest that when there is no difference among dprime and 0 (in other words, the subject did not process counciously the prime). The anova analysis reveals that there are evidence to claim a Relation effect (F(2,40) = 3.72, p = 0.032) priming effect between WR over NR ( p = 0.042) and a marginal significance among  SR over NR (p = 0.086)
