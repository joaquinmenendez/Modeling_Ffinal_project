...
Hay diferencia entre dprime por grupo F(3,96)=22.29 p < .001.
...
Variables are homogeneous and groups are normal
...
Prelikert by SOA groups are homogenous (Levene test  F = 0.1087, p = 0.9548) but no normal (Shapiro Wilk tests). Given that reason, I decided to apply a non-parametric Kruskall Wallis. The Kruskall test show a statistical trend ($X^2$= 6.575, p = .079). The Wilcoxoon rank sum test shows that there are no difference between groups.
...
histogramRT<-ggplot(data = df_summarized, aes(x=dprime,colour= SOA, fill=SOA))+
  labs(x="RT")+
  ggtitle("Histogram of dPrime")+
  geom_density() +
  stat_function(fun = dnorm, colour = "Black", args = list(mean=mean(df_summarized$dprime), sd=sd(df_summarized$dprime))) + facet_wrap(~SOA)  + theme_classic()
histogramRT
#Las varianzas son homogeneas entre los grupos pero no muy lindas


histogramRT<-ggplot(data = df_summarized, aes(x=prelikert, color = SOA , fill = SOA))+
  labs(x="RT")+
  ggtitle("Density  of prelikert")+
  geom_density() +
  stat_function(fun = dnorm, colour = "Black", args = list(mean=mean(df_summarized$prelikert), sd=sd(df_summarized$prelikert))) + facet_wrap(~SOA)  + theme_classic()
histogramRT
#Las varianzas son homogeneas entre los grupos pero no muy lindas
```
...
when we checked the normality on the RT variable for the 4 groups we observed that groups are not homogeneous ( Levene test F(3,296)= 8.956, 1.069e-05 ***)
...
histogramRT<-ggplot(df_inv, aes(x=RT, color = SOA , fill = SOA))+
  labs(x="RT")+
  ggtitle("Density plot of Inverse tranformation(RT) ")+
  geom_density( )+
  stat_function(fun = dnorm, colour = "Black", size = 1, args = list(mean=mean(df_inv$RT), sd=sd(df_inv$RT))) + facet_wrap(~SOA)  + theme_classic() 
histogramRT
