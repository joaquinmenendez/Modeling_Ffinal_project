#This is the second  script to run. It calculates the Visibility score in the objective test.
...
visibility  = read.csv(file = fffile, skip = 301)[c(-149:-600),]  #I know the row because they are fixed
...
for (n in seq(21, to =length(visibility[,2]), by=1)){
    if ((as.character(visibility[n,7])== as.character(visibility[n,8])) & (as.character(visibility[n,4]) %in% animales_prime)){
      animales.correctas = animales.correctas + 1
      #suma a la casilla de ese sujeto de animales correctas
      }
    
      #evalua animales incorrectas
    else if ((as.character(visibility[n,7]) != as.character(visibility[n,8]))  & (as.character(visibility[n,4]) %in% animales_prime)){
      animales.incorrectas = animales.incorrectas + 1
      #suma a la casilla de ese sujeto de animales correctas
     }
    
      #evalua objetos correctas
    else if ((as.character(visibility[n,7]) != as.character(visibility[n,8])) & (as.character(visibility[n,4]) %in% objetos_prime)){
      objetos.correctas = objetos.correctas + 1
      #suma a la casilla de ese sujeto de objetos correctas
       }
    
      #evalua objetos incorrectas
    else if  ((as.character(visibility[n,7]) == as.character(visibility[n,8])) & (as.character(visibility[n,4]) %in% objetos_prime)){
...
# La seleccion del HIT/RATE es arbitraria. Dado que no hay un ruido de por si se selecciono responder a animales como correcto
# Y responder a objetos como ruido
#Agregamos nombres de columnas y dividimos la cantidad de animales y objetos  para obtener los HITRATE Y FALSEALARM.Por ultimo Calculamos el DPRIME
...
df$ID = paste0('66_',df$ID[1]) #AGREGADO NUEVO
